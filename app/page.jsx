...
export default function Page() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/candles?symbol=${symbol}`).then(r => r.json()).then(d => setCandleSource(d.source || '...')).catch(() => {});
  }, [symbol]);
  ...
  return (
    <main style={{fontFamily:'system-ui',background:'#07111f',color:'#e3f0ff',minHeight:'100vh',padding:24}}>
      ...
      <div style={{margin:'12px 0',display:'inline-block',padding:'6px 10px',borderRadius:999,border:'1px solid rgba(114,164,255,.18)',background:'rgba(9,24,43,.9)',color:'#8ea9c7'}}>Status: {health}</div>
